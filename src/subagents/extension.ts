  ...
  return function subagents(pi: ExtensionAPI): void {
    ...
    const registerSubagentsTool = (description: string) => pi.registerTool({
      ...
      async execute(toolCallId, params, signal, onUpdate, ctx) {
        ...
        if (background && isWorkerSession(ctx)) throw new Error("a worker's subagents call cannot be background");
        ...
        const backgroundCall = background ? backgroundCalls.start({ callId: toolCallId, progress }) : undefined;
